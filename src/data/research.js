export const researchProjects = [
  {
    ...
    highlights: [
      "AI agents waste compute by thinking equally hard at every step, even trivial ones. CogSpan routes each step to only the depth of reasoning it actually needs.",
      "Extended an existing cognitive routing framework to two completely different task types for the first time: web-based tool use and competition-level math.",
      "Built a budget tracker that watches token spend mid-episode and quietly caps reasoning depth when the agent is burning through its budget too fast.",
      "Matched the accuracy of running 8 parallel solutions while using 3 times fewer tokens, by routing most problems shallow and only escalating when the task demands it.",
      "Designing a follow-on safety layer for AI agents that flags irreversible actions and forces a verification check before the agent can proceed.",
    ],
  ...
  },
  {
    ...
    highlights: [
      "Every AI efficiency paper asks: how do I answer one question with fewer tokens? We asked a harder one: what happens when you have ten questions and a shared budget?",
      "Built an environment where spending too many tokens on an easy problem means failing a hard one later in the same episode.",
      "The reward function punishes overthinking on easy problems and rewards finishing the full episode within budget. Budget-aware behavior only emerged at 14B scale.",
      "Ran distributed training across 8 A100s with a decoupled inference server to scale to the model sizes where the behavior actually appeared.",
      "The environment logged zero errors across all completed runs. Every failure was a distributed infrastructure issue, not a flaw in the idea.",
    ],
  ...
  },
  {
    ...
    highlights: [
      "Can an LLM recover what 1,466 people would have said in a survey, given only 74 responses as a starting point? We tested it on a real COVID-19 misinformation dataset.",
      "Matching the average response is the wrong measure. We built a three-axis framework checking whether relationships, distributions, and individual responses all hold up.",
      "The best model got the population distribution right but could only explain 14% of what made individual people different from each other.",
      "Aggregate accuracy hid major subgroup failures. Accuracy dropped by more than half for conservative respondents, invisible until you look past the overall number.",
      "A classification head on the same fine-tuned model beat text generation across all three fidelity axes, with no change to the base model or training setup.",
    ],
    writtenDate: "Apr 2026",
    readTime: "4 min read",
    coverImage: null,
